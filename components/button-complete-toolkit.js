...
export default function CompleteToolkitButton ({button}){
    ...
    return(
            <div className={`complete-toolkit-btn-stroke-gradient relative
            ...
            `}>
                <span className="block md:hidden leading-[35px]">
                    <img className="inline-block w-[38px] h-auto mr-8" src={button.buttonImage}/>
                    <span className="leading-[21px] tracking-[0.0025em]">{t(button.title)}</span>
                </span>
                <img className="hidden md:block" src={button.buttonImage}/>
                <span className="hidden md:block md:pt-[32px] md:whitespace-pre-wrap text-center leading-[22px] tracking-[0.005em]">{t(button.title)}</span>
            </div>
    )
}